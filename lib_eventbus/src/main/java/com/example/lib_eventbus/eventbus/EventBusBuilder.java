...
import com.example.lib_eventbus.eventbus.android.AndroidLogger;
...
public class EventBusBuilder {
    ...
    boolean logNoSubscriberMessages = true;//不需要订阅者消息日志
    boolean sendSubscriberExceptionEvent = true;//是否发送订阅者异常
    boolean sendNoSubscriberEvent = true;//发送无订阅者异常
    boolean throwSubscriberException;//是否抛出订阅者异常
    boolean eventInheritance = true;//  是否继承事件
    boolean ignoreGeneratedIndex;//是否忽略生成的索引
    boolean strictMethodVerification;//是否严格执行方法的验证
    ExecutorService executorService = DEFAULT_EXECUTOR_SERVICE;//默认线程池
    List<Class<?>> skipMethodVerificationForClasses;//需要跳过方法执行的类集合
    List<SubscriberInfoIndex> subscriberInfoIndexes;//订阅者信息的索引集合
    Logger logger;
